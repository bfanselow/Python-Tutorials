...
Numerous resource-access operations that might appear single-step on the surface are actually composed of multiple steps. In this case the operation is not **thread safe**. Libraries that manage a resource, such as a database, or socket library need to be written with extreme care to be thread safe.  In general, its best to assume that the library is not thread safe.
